...
class Onlinedoffreporta extends CI_Controller {
...
//		$data['categories']=$categories;
		...
		$this->load->view('admin/doffdata/Onlinedoffreporta');
//		$this->load->view('admin/doffdata/doff10report');
    ...
    public function showonlinedoffreporta() {
        $frameNo = $this->input->post('frameNo');
        $doffdate = $this->input->post('doffdate');
    
        // Check for view file existence
        $viewFilePath = APPPATH . 'views/admin/category/delete.php';
...
$response = array(
	...
	public function getfrmshno() {
        $frameNo = $this->input->post('frameNo');
        $companyId = $this->input->post('companyId');
		$frm='';
		$sql="select frame_no from  mechine_master where mech_code='$frameNo'";	
		$bwt=0;
		$query1=$this->db->query($sql);
		if ( $query1->num_rows()>0 ) {
			$row1 = $query1->row();
