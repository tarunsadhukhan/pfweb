...
class Finishing_data_export extends CI_Controller {
	 ...
	 */
	 ...
	 public function __construct() {
        parent::__construct();
        $this->load->model('Finishing_export_data_Model'); // Load the model
    } 
...
//		$data['categories']=$categories;
		...
		$this->load->view('admin/reports/finishing_data_export');
...
public function finishalllist() {
	$date = $this->input->post('date');
	$compid = $this->input->post('companyId');
	$date=substr($date,6,4).'-'.substr($date,3,2).'-'.substr($date,0,2);
    $records=$this->Finishing_export_data_Model->getfinishalldata($date,$compid);
    ...
    $cnt=count($records);
			...
			if ($cnt>0) {
				foreach ($records as $record) {
					$data[] = [
						$record['work_type'],         // Use array notation instead of object property
						$record['entrydate'],       // Use array notation instead of object property
						...
						$record['process_type'],      // Use array notation instead of object property
						...
						$record['noofmc'] ,     // Use array notation instead of object property
						$record['worktype']      // Use array notation instead of object property
					];
				}
		} 
...
public function finishchklist() {
	$date = $this->input->post('date');
	$compid = $this->input->post('companyId');
	$date=substr($date,6,4).'-'.substr($date,3,2).'-'.substr($date,0,2);
    $records=$this->Finishing_export_data_Model->getfinishchkdata($date,$compid);
...
public function exportdbfdata() {
	...
	$date=substr($date,6,4).'-'.substr($date,3,2).'-'.substr($date,0,2);
    $data=$this->Finishing_export_data_Model->getfinishalldata($date,$compid);
    $cnt=count($data);
    $fileContainerhm = "hemmdata.csv";
    $filePointerhm = fopen($fileContainerhm,"w+");
    $logMsghm='';
    $fileContainerhr = "heradata.csv";
    $filePointerhr = fopen($fileContainerhr,"w+");
    $logMsghr='';
    $rowIndex = 4;
	foreach ($data as $row) {
        if  ($row['work_type']==6) {
                $logMsghm.= $row['entrydate'].",".$row['spell'].",".$row['eb_no'].",".$row['process_code'].",".$row['production'].
                ",0,O,".$row['mech_code'].",".$row['whrs']."\r\n";
        }    
        if  ($row['work_type']==7) {
            $logMsghr.= $row['entrydate'].",".$row['spell'].",".$row['eb_no'].",".$row['process_code'].",".$row['production'].
            ",0,O,".$row['mech_code'].",".$row['whrs']."\r\n";
}    
    ...
    fputs($filePointerhm,$logMsghm);
    fclose($filePointerhm);
    fputs($filePointerhr,$logMsghr);
    fclose($filePointerhr);

	$date = $this->input->get('date');
	$compid = $this->input->get('companyId');
	$date=substr($date,6,4).'-'.substr($date,3,2).'-'.substr($date,0,2);
    $data=$this->Finishing_export_data_Model->getfinishhlpdata($date,$compid);
    $cnt=count($data);
    $fileContainerhhm = "hemhdata.csv";
    $filePointerhhm = fopen($fileContainerhhm,"w+");
    $logMsghhm='';
    $fileContainerhhr = "herhdata.csv";
    $filePointerhhr = fopen($fileContainerhhr,"w+");
    $logMsghhr='';
    $rowIndex = 4;
	foreach ($data as $row) {
        if  ($row['dsgid']==106) {
                $logMsghhm.= $row['attendance_date'].",".$row['eb_no'].",".$row['spell'].",".$row['mech_code']."\r\n";
        }    
        if  ($row['dsgid']==109) {
			$logMsghhr.= $row['attendance_date'].",".$row['eb_no'].",".$row['spell'].",".$row['mech_code']."\r\n";
		}    
    ...
    fputs($filePointerhhr,$logMsghhr);
    fclose($filePointerhhr);



    $txt1= $fileContainerhm;
    $txt2= $fileContainerhr;
    $txt3= $fileContainerhhm;
    $txt4= $fileContainerhhr;
