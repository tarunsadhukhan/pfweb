...
class Weaving_data_export extends CI_Controller {
...
//		$data['categories']=$categories;
		...
		$this->load->view('admin/reports/weaving_data_export');
...
$compid = $this->input->get('companyId');
    ...
    $sql = "SELECT DATE_FORMAT(loom_date,'%d-%m-%Y') loomdate ,mm.mech_code, cjb.*,
    production_a1 /finished_length_a1 *16 prda1,production_a2 /finished_length_a2 *16 prda2,
    production_b1 /finished_length_b1 *16 prdb1,production_b2 /finished_length_b2 *16 prdb2,
    production_c/finished_length_c *16 prdc  FROM cuts_jugar_buff_1 cjb 
    left join mechine_master mm on cjb.loom_id =mm.mechine_id 
    where loom_date ='".$sdate."' and cjb.company_id =".$compid." and cjb.is_active =1 and length(mm.mech_code)=6
       ";
    ...
    $rowIndex = 4;
	foreach ($data as $row) {
        $logMsg.= $row['loomdate'].",".$row['mech_code'].",,,,,,,".$row['quality_code_a1'].",".$row['quality_code_a2'].
        ",".$row['quality_code_b1'].",".$row['quality_code_b2'].",".$row['quality_code_c'].
        ",".$row['ticket_no_a1'].",".$row['ticket_no_a2'].",".$row['ticket_no_b1'].",".$row['ticket_no_b2'].",".$row['ticket_no_c'].
        ",".$row['prda1'].",".$row['prda2'].",".$row['prdb1'].",".$row['prdb2'].",".$row['prdc'].
        ",".$row['working_hrs_a1'].",".$row['working_hrs_a2'].",".$row['working_hrs_b1'].",".$row['working_hrs_b2'].",".$row['working_hrs_c']."\r\n";
