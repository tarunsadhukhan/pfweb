...
class Pfuploadfileupd extends CI_Controller {
...
//		$this->load->view('admin/winding_doff/winding_doff_data',$data,$dataq);
		$this->load->view('admin/uandetails/Pfuploadfileupd', $data_to_pass);	
	}
 ...
 */
 ...
 //        $query = $this->db->query($sql);
    ...
    public function update_monthpfdata() {
      ...
      //  $pfgendate=substr($pfgendate,6,4).'-'.substr($pfgendate,3,2).'-'.substr($pfgendate,0,2);
       ...
       // record_id: record_id,companyId: companyId, ac1amt: ac1amt,ac2amt: ac2amt,ac10amt: ac10amt,
      //  ac21amt: ac21amt,ac22amt : ac22amt,totamt : totamt,upshare : upshare
 
        $record_id = $this->input->post('record_id');
        $trrnno = $this->input->post('trrnno');
        $chaldate = $this->input->post('chaldate');
        $ac1amt = $this->input->post('ac1amt');
        $ac2amt = $this->input->post('ac2amt');
        $ac10amt = $this->input->post('ac10amt');
        $ac21amt = $this->input->post('ac21amt');
        $ac22amt = $this->input->post('ac22amt');
        $totamt = $this->input->post('totamt');
        $upshare = $this->input->post('upshare');
        $paydate = $this->input->post('pydate');
        

        $pfgendate = $this->input->post('chaldate');
        $chaldate=substr($pfgendate,6,4).'-'.substr($pfgendate,3,2).'-'.substr($pfgendate,0,2);
        if (strlen($chaldate)<5) {
            $chaldate=NULL;
        }

        $pfgendate = $this->input->post('paydate');
        $paydate=substr($pfgendate,6,4).'-'.substr($pfgendate,3,2).'-'.substr($pfgendate,0,2);
        if (strlen($paydate)<5) {
            $paydate=NULL;
        }
        $active=1;
    ...
    //    $sql="update EMPMILL12.tbl_pf_generation set is_active=0 where company_id=".$companyId." and month_end_date='".$pfgendate."'";
//echo 'chal=='.$chaldate;
//   console.log($chaldate);
        $data = array(
            'trrn_no' => $trrnno,
            'trrn_status' => $upshare,
            'ac_2_amount' => $ac2amt,
            'ac_21_amount' => $ac21amt,
            'ac_22_amount' => $ac22amt,
            'trrn_amount' => $totamt,
            'is_active' => $active
         
            // Exclude 'id' and 'updated_by' fields
        );   
        $this->db->where('pf_hdr_upload_id', $record_id);
        $this->db->update('EMPMILL12.tbl_pf_hdr_upload_data', $data);

        $sql="update EMPMILL12.tbl_pf_hdr_upload_data set challan_date='".$chaldate."',payment_date='".$paydate."'  
        where pf_hdr_upload_id=".$record_id;
    // if ((strlen($chaldate)<5) && (strlen($paydate)<5)) {
     if  ($chaldate === null && $paydate === null ) {
        $sql="update EMPMILL12.tbl_pf_hdr_upload_data set challan_date=null ,payment_date=null where pf_hdr_upload_id=".$record_id;
     } 
     if ($chaldate ===null  && $paydate !==null ) {
        $sql="update EMPMILL12.tbl_pf_hdr_upload_data set challan_date=null ,payment_date='".$paydate."' 
      where pf_hdr_upload_id=".$record_id;
  } 
  if ($chaldate !==null  && $paydate ===null ) {
    $sql="update EMPMILL12.tbl_pf_hdr_upload_data set challan_date='".$chaldate."',payment_date=null 
  where pf_hdr_upload_id=".$record_id;
} 
        $this->db->query($sql);
    
        $sql="update EMPMILL12.tbl_pf_line_upload_data set is_active=".$actv." where pf_hdr_upload_id=".$record_id;
...
public function Pfupddatalast() {
    ...
    $compid = $this->session->userdata('company_id');
     ...
     when  trrn_status=2 then 'EE Share'
     when  trrn_status=3 then 'ER Share'
     when  trrn_status=4 then 'No Share' end trrn_stat_type
     from EMPMILL12.tbl_pf_hdr_upload_data tphud where month_end_date in (
     select max(month_end_date) from EMPMILL12.tbl_pf_hdr_upload_data tphud where is_active=1 and company_id=".$compid.")"; 
     $query = $this->db->query($sql );
     $records = $query->result();
     $sln=$query->num_rows();
     ...
     foreach ($records as $record) {
         $data[] = [
//            if(strlen($record->payment_date)>0),
  //          $record->challan_date,
...
{
    $batchno = trim((string)$this->input->get('batchno'));   // e.g. "12,13,14"
    $compid  = $this->input->get('companyId');
    $oldnew  = $this->input->get('oldnew');
    // Parse batch numbers safely
    $numbers = array_values(array_filter(array_map(function($v){
        $v = trim($v);
        return ($v === '' ? null : (int)$v);
    }, explode(',', $batchno)), function($v){ return $v !== null; }));

    if (empty($numbers)) {
        show_error('No valid batch numbers provided.');
        return;
    }

    // Get header rows
    $this->db
        ->select("tphud.*, CONCAT(UCASE(SUBSTRING(MONTHNAME(month_end_date),1,3)), DATE_FORMAT(month_end_date,'%Y'), '_', batch_process_no) AS flname,ifnull(main,0) main", false)
        ->from('EMPMILL12.tbl_pf_hdr_upload_data AS tphud')
        ->where('is_active', 1)
        ->where_in('batch_process_no', $numbers);
...
//var_dump($data);
    if (empty($data)) {
        show_error('No PF header records found for the given batches.');
        return;
    }

    // Prepare output directory
    $dir = rtrim(FCPATH, '/\\') . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'pf' . DIRECTORY_SEPARATOR;
    if (!is_dir($dir)) {
        if (!@mkdir($dir, 0777, true) && !is_dir($dir)) {
            log_message('error', "PF Upload: Failed to create dir: {$dir}");
            show_error("Cannot create directory: {$dir}");
            return;
        }
    }
    if (!is_writable($dir)) {
        log_message('error', "PF Upload: Directory not writable: {$dir}");
        show_error("Directory not writable: {$dir}");
        return;
    }
    ...
    foreach ($data as $row) {
        $fl    = $row['flname'];
        $hdrid = (int)$row['pf_hdr_upload_id'];
        $mup   = $row['main'];

        // Normalize date
        try {
            $mndt = $row['month_end_date'];
        } catch (Exception $e) {
            log_message('error', 'PF Upload: Invalid month_end_date for hdrid ' . $hdrid . ' -> ' . ($row['month_end_date'] ?? 'NULL'));
            // Skip this header row rather than crash
            continue;
        }

        // Fetch line rows for this header
        $this->db->select('tplud.*, tum.uan_no AS uanno, tum.name_as_per_pf_online, 0 AS refund', false)
                 ->from('EMPMILL12.tbl_pf_line_upload_data AS tplud')
                 ->join('EMPMILL12.tbl_uan_master AS tum', 'tum.uan_id = tplud.uan_id', 'left')
   ...
   //     echo $mndt.'--'.$cutoff.'--'.$mup.'--'.$oldnew;
    ...
    //        echo 'cur mnth '.$mndt.'--'.'cut off '.$cutoff.'--'.'myp '.$mup.'--'.'oldnew '.$oldnew.'--uan '.$lrow['uanno'].'<br>';
     ...
     //       echo 'cur mnth '.$mndt.'--'.'cut off '.$cutoff.'--'.'myp '.$mup.'--'.'oldnew '.$oldnew.'--uan '.$lrow['uanno'].'<br>';
                ...
                if ( ($mndt>= $cutoff) && $mup==9 && $oldnew==2) {
                // Minimal format (catch-all)
                $logMsg .= $lrow['uanno'] . $sp . $lrow['name_as_per_pf_online'] . $sp . $lrow['epf_contribution']
                        .  $sp . $lrow['eps_contribution'] . $sp . $lrow['epf_eps_diff_contribution']
                        .  $sp . $lrow['refund'] . "\r\n";
                }   
...
//            echo $logMsg;
        // (Optional) add batch numbers at end of file
        if (!empty($numbers)) {
//            $logMsg .= implode(',', $numbers) . "\r\n";
        }

        // Write file safely
        $fileContainer = $dir . $fl . '.txt';
        $bytes = @file_put_contents($fileContainer, $logMsg, LOCK_EX);
        if ($bytes === false) {
            $err = error_get_last();
            log_message('error', "PF Upload: file_put_contents failed for {$fileContainer}: " . ($err['message'] ?? 'unknown'));
            show_error("Unable to write file: {$fileContainer}");
            return;
        }
    ...
    // You can return/echo the file list, or render a view to show downloads
