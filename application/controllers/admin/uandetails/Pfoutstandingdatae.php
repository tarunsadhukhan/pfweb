...
class Pfoutstandingdatae extends CI_Controller {
...
$query = $this->db->query($sql,$upfromdate,$uptodate,$compid,$compid );
        ...
        GROUP BY 
          ...
          $sql="select k.month_end_date monname,empl_cont gac1amt,emplo_cont gac10amt,gadmchgs,
          ifnull(empl_pay,0) pac1amt,ifnull(emplo_pay,0) pac10amt
          ,ifnull(adm_pay,0) padmchgs,ifnull(aempl_pay,0) aempl_pay,ifnull(aempl_pay,0) aemplo_pay,
          ifnull(aadm_pay,0) aadm_pay,
          (empl_cont+emplo_cont+gadmchgs-ifnull(empl_pay,0)-ifnull(emplo_pay,0)-ifnull(adm_pay,0)) oustamt from 
          (	
          select tpg.month_end_date,sum(epf_contibution) empl_cont,sum(eps_contribution+epf_eps_diff_contribution) emplo_cont,
          round(sum(epf_contibution)*10/100,0) gadmchgs 
      ...
      SELECT 
          ...
          SUM(CASE 
                  ...
                  WHEN tphud.trrn_type = 3 THEN tphud.ac_10_amount + tphud.ac_1_amount
              END) AS aemplo_pay,
          SUM(tphud.ac_2_amount + tphud.ac_21_amount + tphud.ac_22_amount) AS aadm_pay
      FROM 
          EMPMILL12.tbl_pf_hdr_upload_data tphud 
      WHERE 
          tphud.is_active = 1 and month_end_date between '".$upfromdate."' and '".$uptodate."'
          and company_id=".$compid." and tphud.trrn_status in (2,3)
      GROUP BY 
          tphud.month_end_date
          ) gm on gm.month_end_date=k.month_end_date
          left join
          (
      SELECT 
          tphud.month_end_date,
...
//            $sheet->setCellValue('A1', 'For the Month of '.$sdate);
        //	$active_sheet->setCellValue('A2', "Reports for Dated : ");
            ...
            $sheet->setCellValue('f1', 'PF Uploaded');
            $sheet->setCellValue('j1', 'PF Payment');
            $sheet->setCellValue('n1', 'Outstanding(Ded)');
            $sheet->setCellValue('o1', 'Outstanding(Upd)');
 
            $sheet->setCellValue('A2', 'Month End Date ');
            $sheet->setCellValue('b2', 'Employee Cont');
            $sheet->setCellValue('c2', 'Employer Cont');
            $sheet->setCellValue('d2', 'Adm Charges');
            $sheet->setCellValue('e2', 'Total Amount');
            
            $sheet->setCellValue('f2', 'Employee Cont');
            $sheet->setCellValue('g2', 'Employer Cont');
            $sheet->setCellValue('h2', 'Adm Charges');
            $sheet->setCellValue('i2', 'Total Amount');


            $sheet->setCellValue('j2', 'Employee Cont Paid');
            $sheet->setCellValue('k2', 'Employer Cont Paid');
            $sheet->setCellValue('l2', 'Adm Charges Paid');
            $sheet->setCellValue('m2', 'Total Amount');
...
//            $sheet->setCellValue('j2', 'Outstanding Amount');
            ...
            foreach ($records as $record) {
                ...
                $grout=$grout+$record->oustamt;       
                    ...
                    $value=$record->aempl_pay;
                    $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex, $value);
                    $columnIndex++;
                    $value=$record->aemplo_pay;
                    $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex, $value);
                    $columnIndex++;
                    $value=$record->aadm_pay;
                    $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex, $value);
                    $columnIndex++;
                    $value=$record->aempl_pay+$record->aemplo_pay+$record->aadm_pay;
                    $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex, $value);
                    $columnIndex++;
                    ...
                    $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex, $value);
                    $columnIndex++;
                    $value=($record->aempl_pay+$record->aemplo_pay+$record->aadm_pay)-($record->padmchgs+$record->pac1amt+$record->pac10amt);
                    $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex, $value);
                $rowIndex++;
            }	
            $columnIndex = 1;
            ...
            $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex, $value);
            $columnIndex++;
            $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex(2); // Convert column index to letter
            $sumRange = $columnLetter . '3:' . $columnLetter . $rowIndex-1; // Define the range (e.g., C3:C10)
            $sheet->setCellValueByColumnAndRow(2, $rowIndex , "=SUM($sumRange)"); 
            $columnIndex++;
            $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($columnIndex); // Convert column index to letter
            $sumRange = $columnLetter . '3:' . $columnLetter . $rowIndex-1; // Define the range (e.g., C3:C10)
            $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex , "=SUM($sumRange)"); 
            $columnIndex++;
            $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($columnIndex); // Convert column index to letter
            $sumRange = $columnLetter . '3:' . $columnLetter . $rowIndex-1; // Define the range (e.g., C3:C10)
            $sheet->setCellValueByColumnAndRow($columnIndex, $rowIndex , "=SUM($sumRange)"); 
