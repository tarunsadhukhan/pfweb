...
class Pfuploadfilegen extends CI_Controller {
    ...
    public function gen_createpfupdata() {
        ...
        $epfcont = $summary->epf_contribution + $summary->epf_eps_diff_contribution ;
        //+ round($summary->eps_contribution * 1.67 / 10, 0);
        //round($summary->eps_contribution * 1.67 / 10, 0);
        $epscont =$summary->eps_contribution;
//        echo var_dump($summary);
//        echo 'TRN AMT: ' . $trnamt . "\n";
//        echo 'EPF CONT: ' . $epfcont . "\n";
    ...
    public function gen_monthpfupdata() {
        ...
        while ($current_date <= $end_date_obj) {
         ...
         //   echo 'Current Header ID: ' . $curhdrid . "\n";
            ...
            $trnamt = $summary->pfcont + $summary->admchgs;
            $epfcont = $summary->epf_contribution + $summary->epf_eps_diff_contribution ;
            //+ round($summary->eps_contribution * 1.67 / 100, 0);
            $epscont = $summary->eps_contribution;
//            echo 'TRN AMT: ' . $trnamt . "\n";
//            echo 'EPF CONT: ' . $epfcont . "\n";
//            echo 'EPS CONT: ' . $epscont . "\n";
            if ($trnamt > 0) {
                $this->Pfupload_Model->updatePfHdrAmounts($maxpfhdrid, $trnamt, $epfcont, $epscont, $summary->admchgs);
            } else {
                $this->Pfupload_Model->updatePfHdrStatusInactive($maxpfhdrid);
    ...
    function genpfup11($companyId, $pfmonth, $pfyear, $pfdays, $gdate, $upshare, $maxpfhdrid) {
        $this->Pfupload_Model->insertPfLineAllOld($maxpfhdrid, $gdate, $companyId,$upshare);
    }
    ...
    function genpfup111($companyId, $pfmonth, $pfyear, $pfdays, $gdate, $upshare, $maxpfhdrid, $pfprvdate, $curhdrid) {
        if ($gdate <= $pfprvdate) { 
            ...
            $this->Pfupload_Model->insertPfLineAllNew($maxpfhdrid,$gdate, $curhdrid, $upshare);
        }
    }

    function genpfup12($companyId, $pfmonth, $pfyear, $pfdays, $gdate, $upshare, $maxpfhdrid, $pfprvdate, $curhdrid) {
        if ($gdate <= $pfprvdate) {
            $this->Pfupload_Model->insertPfLineEpfOnlyOld($maxpfhdrid, $gdate, $companyId, $upshare);
        } else {
            $this->Pfupload_Model->insertPfLineEpfOnlyNew($maxpfhdrid, $gdate, $curhdrid, $upshare);
        }
    }

    function genpfup13($companyId, $pfmonth, $pfyear, $pfdays, $gdate, $upshare, $maxpfhdrid, $pfprvdate, $curhdrid) {
        if ($gdate <= $pfprvdate) {        
            $this->Pfupload_Model->insertPfLineEpsOnlyOld($maxpfhdrid, $gdate, $companyId, $upshare);
        } else {
            $this->Pfupload_Model->insertPfLineEpsOnlyNew($maxpfhdrid, $curhdrid, $upshare, $gdate);
        }
    }
